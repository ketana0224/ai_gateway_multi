...
Portal → `apim-aigw-handson-<id>` → 左メニュー **監視 > Application Insights**
...
Portal → `apim-aigw-handson-<id>` → 左メニュー **APIs > API** → 中央 API 一覧の一番上 **`All APIs`** をクリック → 右ペイン上部タブ **Settings**
...
flowchart LR
    ...
    Ingress -.->|"span 発行"| R["<b>AppRequests</b><br/>AppRoleName=apim-aigw-handson-...<br/>OperationName=ChatCompletions_Create"]
    Egress -.->|"span 発行<br/>(HTTP メタデータのみ)"| D["<b>AppDependencies</b><br/>AppRoleName=apim-aigw-handson-...<br/>Name=POST .../chat/completions<br/>Target=&lt;aoai endpoint&gt;<br/>DurationMs / ResultCode<br/><i>※ gen_ai.* 属性は付かない</i>"]
...
5. **`AppRoleName` が `apim-aigw-handson-<id>` の行が 2 行返り、かつ親子関係が成立していれば成功** です。期待される並び:
   ...
   | **② バックエンド呼び出し (Egress = LLM Call)** | `<span-id-B>` | **`<span-id-A>`** ← AppRequests の Id を指す | `POST /openai/deployments/.../chat/completions` | `aif-aigw-handson-<id>.openai.azure.com` | ~1221 ms |
...
>
> :warning: **Application Insights への接続は Azure Portal ではなく Foundry ポータル側で行います**。Foundry リソース (`aif-aigw-handson-<id>`) の Azure Portal サイドメニュー（監視配下）には **警告 / メトリック / 診断設定 / ログ しかなく、「Application Insights」項目は存在しません**。

### 手順

1. Azure Portal → `aif-aigw-handson-<id>`（Lab 2 で作成した Foundry リソース）を開く
...
> | 用語 | 実体 |
> |---|---|
> | **cloud APIM**（マネージド ゲートウェイ） | `apim-aigw-handson-<id>` の APIM 本体に組み込まれた既定ゲートウェイ。Azure 内で Microsoft が運用 |
...
3. **SHGW の AppRoleName を確認**（cloud APIM と異なる値になる）:
   ...
   | AppRoleName | 説明 |
   |---|---|
   | `apim-aigw-handson-<id>` | cloud APIM（マネージド ゲートウェイ）|
   | `apim-aigw-handson-<id> <gateway-resource-name>` | SHGW（例: `apim-aigw-handson-user99 aws-ap-northeast-1`）|
...
4. **SHGW のリクエスト span を確認**:
   ...
   AppRequests
   | where TimeGenerated > ago(10m)
   | where AppRoleName != "apim-aigw-handson-<id>"   // cloud APIM を除外。SHGW は名前にスペースが入る
   | project TimeGenerated, OperationId, OperationName, Url, ResultCode, DurationMs, AppRoleName
   | order by TimeGenerated desc
   | take 5
   ```
