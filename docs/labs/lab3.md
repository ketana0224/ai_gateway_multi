...
>
> :information_source: **試したい場合の進め方**: 公式ドキュメント手順に沿って Foundry portal `操作 → 管理者 → AI Gateway` タブで AI Gateway を新規作成（Basic v2 / Free レベル枠内）し、`proj-default-<id>` を **プロジェクトをゲートウェイに追加** で有効化してください。本ラボの 3-1B 以降には影響しません。

### 3-1B. APIM Portal で Foundry のモデルを API リソース として公開

> :rotating_light: **【受講者必読】このウィザードは作成時に **必ず** ロール割り当てを新規書き込みします（既存割り当てはチェックしません）。本ハンズオンでは受講者に `Microsoft.Authorization/roleAssignments/write` 権限が無いため、そのまま実行すると **「API を作成できませんでした」エラーで API 作成自体がロールバックされます**。
>
> **ウィザードを起動する前に**、講師が以下 2 つを完了しているか確認してください（未完了なら依頼）:
>
> 1. **APIM `apim-aigw-handson-<id>` の System-assigned Managed Identity** に対し、Foundry リソース `aif-aigw-handson-<id>` スコープで **`Cognitive Services User`** ロール付与（実行時に APIM が Foundry を呼ぶために必要）
> 2. **受講者 UPN 自身** に対し、同じ `aif-aigw-handson-<id>` スコープで **`User Access Administrator`** ロール付与（ウィザードが受講者の権限でロール割り当てを書くために必要。**スコープはこの Foundry リソースのみ**に限定し、Lab 3 完了後にこの割り当ては削除してかまいません）
>
> 2 つ両方揃った状態で、受講者はウィザードを実行できます。ウィザードは「另リソースを定義する」警告を出さずに成功し、APIM MI に `Cognitive Services OpenAI User` を追加します（既存の `Cognitive Services User` と並存、両方付いていて問題なし）。
>
> エラーになった場合 → 上記 2. が未付与ということ。**API は何も作られていません**。講師に依頼し、約 5 分待機してからウィザードをもう一度最初から実行してください（APIs 一覧に中途半端な `openai-api` が残っていれば削除）。
>
> <details>
> <summary>講師向け: 受講者番号をループして 1 ＋ 2 を一括付与する PowerShell スニペット</summary>
>
> ```powershell
> # 例: user01 〜 user10 に対して
> #   - APIM MI へ Cognitive Services Userを Foundry スコープで付与
> #   - 受講者 UPN へ User Access Administratorを Foundry スコープで付与（ウィザード完走のため）
> $sub    = (az account show --query id -o tsv)
> $domain = 'M365CPI65139919.onmicrosoft.com'  # テナントドメインに置換
> 1..10 | ForEach-Object {
>   $id      = 'user{0:D2}' -f $_
>   $rg      = "rg-aigw-handson-$id"
>   $apim    = "apim-aigw-handson-$id"
>   $foundry = "aif-aigw-handson-$id"
>   $upn     = "$id@$domain"
>   $scope   = "/subscriptions/$sub/resourceGroups/$rg/providers/Microsoft.CognitiveServices/accounts/$foundry"
>
>   # 1. APIM MI へ Cognitive Services User
>   $miOid = az apim show -g $rg -n $apim --query identity.principalId -o tsv
>   if ($miOid) {
>     az role assignment create --assignee-object-id $miOid --assignee-principal-type ServicePrincipal `
>       --role 'Cognitive Services User' --scope $scope | Out-Null
>   } else {
>     Write-Warning "$id: APIM MI 未取得 (System-assigned 未有効か)"
>   }
>
>   # 2. 受講者 UPN へ User Access Administrator (Foundry スコープのみ)
>   az role assignment create --assignee $upn `
>     --role 'User Access Administrator' --scope $scope | Out-Null
>
>   Write-Host "$id done"
> }
> ```
...
- **APIM のマネージド ID** に `Cognitive Services OpenAI User` ロールが Foundry リソースで自動付与される（事前付与済みの `Cognitive Services User` と並存）
- 認証は MI で完結（API キーを APIM 側で保持しない）

#### Portal 手順

1. `APIM (apim-aigw-handson-<id>) → 左メニュー APIs → + API の追加`
2. **「Create an AI API」** カテゴリ内の **Microsoft Foundry** カードを選択

#### Select AI Service タブ

- **サブスクリプション**: Foundry リソースがあるサブスクリプション
- **Microsoft Foundry**: `aif-aigw-handson-<id>` を選択
...
- Backend `openai-api-ai-endpoint`（URL = `https://<aif-aigw-handson-<id>>.<region>.cognitiveservices.azure.com/openai`、認証 = `Azure resource (Managed identity)`）
- API スコープ inbound policy に `set-backend-service`（`backend-id="openai-api-ai-endpoint"`）と `llm-token-limit` が挿入される（実ポリシーは 3-2 で確認）
- Foundry リソースに `Cognitive Services OpenAI User` ロールが APIM の MI へ追加付与される（事前付与済みの `Cognitive Services User` と並存して両方付いている状態になります）

> :information_source: もし「**API を作成できませんでした**」エラーが出た場合 = §3-1B 冒頭の事前付与 2 番（受講者 UPN への `User Access Administrator`）が未実施です。**API リソースは作成されていません**。講師に付与を依頼し、約 5 分待機してからウィザードをもう一度最初から実行してください。APIs 一覧に中途半端な `openai-api` が残っている場合は削除してからリトライします。
...
> :information_source: `backend-id="openai-api-ai-endpoint"` は Foundry API ウィザードが **APIM の Backends に自動登録**するリソースです（URL = Foundry エンドポイント、認証 = Managed identity）。APIM → Backends で確認できます。
...
```pwsh
$APIM = "https://apim-aigw-handson-<id>.azure-api.net"   # APIM ホスト + openai-api の Base path
$KEY  = "<3-3 で取得した主キー>"
...
- [ ] （任意・3-1A 参考実施時のみ）Foundry portal の **管理者 → AI Gateway** タブで Basic v2 APIM を新規作成し、`proj-default-<id>` をゲートウェイに追加した
- [ ] **Microsoft Foundry** ウィザードで `openai-api` を作成し、Foundry の gpt-4o-mini と接続した
- [ ] APIM の MI に Foundry リソースの `Cognitive Services User` ロールが付与されている（講師が代行付与）
