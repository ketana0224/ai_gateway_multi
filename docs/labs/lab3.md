...
- **Microsoft Foundry**: `aif-aigw-<id>` を選択
...
- Backend `openai-api-ai-endpoint`（URL = `https://<aif-aigw-<id>>.<region>.cognitiveservices.azure.com/openai`、認証 = `Azure resource (Managed identity)`）
...
> :warning: Foundry リソースへのロール付与権限（**所有者** or **ユーザー アクセス管理者**）が必要です。権限不足のときは Wizard が警告するので、講師に依頼するか手動で `APIM の System-assigned MI` に `Cognitive Services OpenAI User` を `aif-aigw-<id>` リソースで割り当ててください。
...
```pwsh
$APIM = "https://apim-aigw-<id>.azure-api.net"   # APIM ホスト + openai-api の Base path
$KEY  = "<3-3 で取得した主キー>"
