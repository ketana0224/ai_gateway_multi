...
5. トークン欄のすぐ下にある **構成エンドポイント** フィールドに `apim-aigw-<id>.configuration.azure-api.net` が表示される（`config.service.endpoint` に使う値）
6. さらに下の **デプロイ スクリプト** セクション → **Docker** タブ → **環境 (env.conf)** ブロックに:
   ```
   config.service.endpoint=apim-aigw-<id>.configuration.azure-api.net
...
[Info] [EventSnapshotRestored], message: revision: 00000xxx, serviceName: apim-aigw-<id>, gatewayId: ..., source: ConfigurationRepositoryProvider
...
| 構成 URL | `apim-aigw-<id>.configuration.azure-api.net` |
