...
> **⚠️ この手順は管理者アカウント（ラボ 1〜6 で使っていた普段のアカウント）で実施してください。**  
> rls-test アカウントではサブスクリプションが見えません（まだ権限がないため）。  
> rls-test アカウントを使うのは次の EL1-5 の検証ステップのみです。

作成したカスタムロールを、ABAC 条件付きでテストユーザーに割り当てます。

> **⚠️ 割り当てスコープは必ずサブスクリプションにしてください。**  
> ワークスペースや RG スコープで割り当てると `Microsoft.Resources/subscriptions/resources/read` が  
> サブスクリプションスコープで有効にならず、Portal アクセス時に 401 になります。  
> 以前ワークスペーススコープで割り当てていた場合は、先に削除してください:  
> `log-aigw-<id>` → **アクセス制御 (IAM)** → **ロールの割り当て** → `rls-test` の行を削除
...
5. **条件** タブ → **「＋ 条件を追加する」** をクリック  
   （条件欄に **「なし」** と表示されている場合はリンクが表示されています）
...
13. 式 #1 と式 #2 の間の **AND / OR** ラジオボタンで **「AND」** を選択（デフォルトは OR のため要変更）
...
17. **「割り当てのタイプ」** タブ:
    - **割り当てのタイプ**: **「アクティブ」** を選択（「対象」は PIM 用のため不要）
    - **割り当て期間**: **「Permanent」** を選択
18. **「レビューと割り当て」** をクリック
...
> **⚠️ 注意**: `LAQueryLogs` は **診断設定を有効化した後** に実行されたクエリのみ記録されます。  
> EL1-5 の実行前に診断設定が未設定だった場合は、下記「診断設定の確認と有効化」を完了してから  
> **rls-test アカウントで EL1-5 の KQL を再度実行**してください。

### 診断設定の確認と有効化

1. `log-aigw-<id>` → **監視** → **診断設定** を開く
2. `audit` を含む設定が存在しない場合:
   - **＋ 診断設定の追加** をクリック
   - **診断設定の名前** に任意の名前を入力（例: `diag-querylog`）
   - **ログ** セクション → **カテゴリ グループ**: **`audit`** にチェック  
     （`Audit` カテゴリが含まれ、これが `LAQueryLogs` テーブルに書き込む）
   - **宛先の詳細** → **Log Analytics ワークスペースへの送信** にチェック  
     → サブスクリプションと `log-aigw-<id>` を選択
   - **保存**
