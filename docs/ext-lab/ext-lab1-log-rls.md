...
| 現在の値 | 対応 |
|---|---|
| ワークスペースのアクセス許可が必要 | そのまま進む ✅ |
| リソースまたはワークスペースのアクセス許可を使用する | 下記手順で変更する |

**変更手順（必要な場合のみ）**:
1. **アクセス制御モード** の横にある **「クリックして変更」** リンクをクリック
2. 表示されたパネルで **「ワークスペースのアクセス許可が必要」** を選択して **OK** をクリック
...
> **⚠️ 重要**: 条件設定画面では「**ビジュアル エディター**」を使用します。  
> コードエディターでも設定可能ですが、構文エラーが発生しやすいため初回はビジュアルエディターを推奨します。
...
#### 式の追加（AppRoleName の制限）

10. **And** を選択して **式の追加** をクリック
11. 以下の値を設定:
    ...
    | キー | `AppRoleName` |
    | 演算子 | **StringEquals** |
    | 値 | `apim-aigw-<id> <gwName>`（Lab 6 §6-3 で確認した SHGW の AppRoleName を入力） |

    > **ℹ️ 例**: `apim-aigw-user99 aws-ap-northeast-1`  
    > Lab 6 §6-3 の KQL `AppRequests | summarize count() by AppRoleName` で確認した値を使用してください。

12. 式 1 と式 2 を両方選択 → **グループ** をクリック（AND グループとして括る）

#### 確認

13. 条件がコード形式で以下のように表示されることを確認:
...
4. Azure Portal → `log-aigw-<id>` → **ログ** を開く
5. 以下の KQL を順に実行して結果を確認する

### ログイン方法 B: Azure CLI（PowerShell）

```powershell
# テストユーザーとして CLI ログイン（別ウィンドウで実行）
az login --use-device-code --allow-no-subscriptions
# ブラウザでデバイスコードを入力し rls-test@<domain> でサインイン
```

---

### 検証 KQL 1: AppRequests の全件確認
...
### 検証 KQL 2: AppDependencies へのアクセス試行
