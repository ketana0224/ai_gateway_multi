...
1. **他ベンダー LLM（AWS Bedrock の Anthropic Claude）への接続**
   実 AWS を使わず、Azure Container Apps 上に AWS Bedrock Runtime 互換の mimic を配置
...
合計 **約 6 時間**（mimic は講師事前準備済み。Lab 1 の APIM プロビジョニング待ち時間に Lab 2 の Foundry リソース作成 + gpt-4o-mini デプロイ + mimic 疎通確認を並行で実施して吸収）
...
| Container Apps (mimic) | **参加者不要** | 講師が事前デプロイした共有エンドポイントを利用。 |
...
| Azure OpenAI | **不使用** | mimic で代替し課金回避。 |
...
| [Lab 2](./labs/lab2.md) | Microsoft Foundry リソース作成 + gpt-4o-mini デプロイ + 講師配布 mimic の疎通確認 | 30 分 |
...
└──────────────────────────────┘
        ...
        ▼                          ▼
┌──────────────────────────┐   ┌──────────────────────────────┐
│ mimic (講師事前デプロイ)    │   │ Azure Container Apps         │
│  rg: rg-mcp-instructor   │   │  rg: rg-aigw-handson-<init>  │
│ Container App            │   │  image: apim-gateway:2.5.0   │
│  AWS Bedrock Runtime     │   │  = APIM Self-hosted Gateway │
│  互換 mimic (Anthropic)   │   │  （AWS Fargate と同じ image）│
└──────────────────────────┘   └──────────────────────────────┘
        │                          │
        ▼                          ▼
...
| mimic Base URL（講師配布） | `<MIMIC_BASE_URL>` |
