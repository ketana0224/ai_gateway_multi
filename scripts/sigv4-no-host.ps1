...
# Host set-header を削除した SigV4 改良版（set-backend-service が backend host を自動付与）
$full = @'
<policies>
    <inbound>
        <base />
...
Write-Host "SigV4 without Host set-header applied. Sleeping 8s..."
...
'{"messages":[{"role":"user","content":[{"text":"hello via SigV4 no Host header"}]}],"inferenceConfig":{"maxTokens":64}}' | Set-Content -Path $tmp -Encoding utf8 -NoNewline
